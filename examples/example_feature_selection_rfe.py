"""Backward feature selection with RFE (Recursive Feature Elimination).

Starts with all features, repeatedly removes the least important one (based
on model coefficients) and refits, until half of the features remain.
Evaluated with cross-validation, so there is no data leakage.
"""
import numpy as np
